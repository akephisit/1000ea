...
input int      InpStepPoints           = 300;        // Default distance between Upper/Lower in points
// Dynamic Step (Auto ATR)
input bool     InpUseAutoAtrStep       = true;       // Use Dynamic Step = ATR * Multiplier
input int      InpAutoAtrPeriod        = 14;         // ATR period for dynamic step
input double   InpAutoAtrMultiplier    = 1.0;        // Multiplier for dynamic step (ATR * multiplier)
...
input int      InpSidewayMinATRPoints  = 0;          // If ATR(points) < this => sideway (0 disable)
input int      InpSidewayEnterBars     = 3;          // Sideway entry requires ATR < MinATR for N consecutive bars
input double   InpSidewayExitMultiplier= 1.3;        // Sideway exit requires ATR > MinATR * Multiplier
...
int      gAtrSidewayHandle = INVALID_HANDLE;
...
bool SaveState()
{
   ...
   GlobalVariableSet(GVName("psideway"), gPausedBySideway ? 1.0 : 0.0);
   return true;
}
...
bool LoadState()
{
   ...
   if(GlobalVariableCheck(GVName("psideway"))) gPausedBySideway = (GlobalVariableGet(GVName("psideway")) > 0.5); else gPausedBySideway=false;
...
//====================================================================
// ATR Base + Dynamic Step + Anti-sideway
//====================================================================
bool EnsureAtrHandles()
{
   bool ok = true;
   if(InpSidewayATRPeriod > 0 && gAtrSidewayHandle == INVALID_HANDLE)
   {
      gAtrSidewayHandle = iATR(_Symbol, PERIOD_CURRENT, InpSidewayATRPeriod);
      if(gAtrSidewayHandle == INVALID_HANDLE)
      {
         if(InpDebugPrint) Print("Failed to create Sideway ATR handle. err=", GetLastError());
         ok = false;
      }
   }
   if(InpUseAutoAtrStep && InpAutoAtrPeriod > 0 && gAtrStepHandle == INVALID_HANDLE)
   {
      gAtrStepHandle = iATR(_Symbol, PERIOD_CURRENT, InpAutoAtrPeriod);
      if(gAtrStepHandle == INVALID_HANDLE)
      {
         if(InpDebugPrint) Print("Failed to create Step ATR handle. err=", GetLastError());
         ok = false;
      }
   }
   return ok;
...
double GetATRPointsForHandle(int handle, int shift)
{
...
{
   if(!InpUseAutoAtrStep || InpAutoAtrPeriod <= 0)
      return InpStepPoints;
      
   if(gAtrStepHandle == INVALID_HANDLE) EnsureAtrHandles();
   
   double atrPts = GetATRPointsForHandle(gAtrStepHandle, 1);
   if(atrPts <= 0)
      return InpStepPoints;
      
   int step = (int)MathRound(atrPts * InpAutoAtrMultiplier);
   if(step < 10) step = 10;
   return step;
}
...
bool IsSidewayNow()
{
   ...
   int checkBars = InpSidewayEnterBars;
   if(checkBars < 1) checkBars = 1;

   double atr[];
   ArraySetAsSeries(atr, true);
   int copied = CopyBuffer(gAtrSidewayHandle, 0, 1, checkBars, atr);
   if(copied <= 0) return gPausedBySideway;

   double currentAtrPts = atr[0] / _Point;

   if(!gPausedBySideway)
   {
      bool enterSideway = true;
      for(int i=0; i<copied; i++)
      {
         if((atr[i] / _Point) >= (double)InpSidewayMinATRPoints)
         {
            enterSideway = false;
            break;
         }
      }
      if(enterSideway && copied == checkBars)
      {
   ...
   {
      double exitThreshold = (double)InpSidewayMinATRPoints * InpSidewayExitMultiplier;
      if(currentAtrPts > exitThreshold)
      {
         gPausedBySideway = false;
      }
   }
   ...
   if(InpDebugPrint && barTime != 0)
      Print("Sideway ATR pts=", DoubleToString(currentAtrPts, 1), " min=", InpSidewayMinATRPoints, " sideway=", (gPausedBySideway?"true":"false"));
...
bool StartSequence()
{
   ...
   if(startSide == SIDE_BUY)
   {
      ...
      gLowerPrice = NormalizeDouble(PriceByPoints(gUpperPrice, stepPts, false), digits);
   }
   else
   {
      gLowerPrice = NormalizeDouble(bid, digits);
      gUpperPrice = NormalizeDouble(PriceByPoints(gLowerPrice, stepPts, true), digits);
   }
...
void OnDeinit(const int reason)
{
   ...
   if(gAtrSidewayHandle != INVALID_HANDLE)
   {
      IndicatorRelease(gAtrSidewayHandle);
      gAtrSidewayHandle = INVALID_HANDLE;
   }
   if(gAtrStepHandle != INVALID_HANDLE)
   {
      IndicatorRelease(gAtrStepHandle);
      gAtrStepHandle = INVALID_HANDLE;
   }
}
